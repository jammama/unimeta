...
import com.learners.unimeta.dto.Result;
...
public class DefaultRestController{
    ...
    DeferredResult<ResponseEntity<?>> successResponse(Page<?> pageableObject) {
        ...
        DeferredResult<ResponseEntity<?>> result = new DeferredResult<>();
        result.setResult(new ResponseEntity<>(Result.success()
                .data(pageableObject.getContent())
                .build(),
                headers,
                HttpStatus.OK));
    ...
    DeferredResult<ResponseEntity<?>> failResponse(Throwable t) {
        ...
        result.setErrorResult(ResponseEntity.ok()
                .body(Result.failWithMessage(t.getMessage())));
        return result;
    }
}
